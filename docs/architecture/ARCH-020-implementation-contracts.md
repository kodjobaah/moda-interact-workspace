...
tool metadata plus exact revisions; base and feature capability members; two releases
and one active pointer; one original persisted grant. IDs are generated by
schema-compatible helpers and returned in a typed fixture object, never parsed for
meaning. Seed explicit unique operation IDs/reasons and current schema/environment
values. No seed on application startup.

Published ARCH-020 revisions/members/audits are intentionally immutable. Therefore
PostgreSQL fixture reset uses a dedicated disposable database guarded as TEST and is
