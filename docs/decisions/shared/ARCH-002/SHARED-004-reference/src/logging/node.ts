...
export function resolveLogsEndpoint(
  ...
  if (signalEndpoint) {
    // Signal-specific endpoint is exact; do not append /v1/logs.
...
function resolveLogsConfig(options: NodeOpenTelemetryLoggingOptions) {
  ...
  if (!endpoint) {
    // Moda deliberately does not use the OTel default localhost endpoint.
    // Local/test environments remain stdout-only unless explicitly configured.
    return {
      enabled: false,
      endpoint: undefined,
      headers: undefined,
