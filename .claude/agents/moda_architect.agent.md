...
ARCHITECT MIRRORED TASK-BRANCH BOUNDARY
===============================================================================
...
Repository tasks use matching branch names in two repositories:

    parent workspace:          task/<TASK_ID>
    implementation repository: task/<TASK_ID>

moda_architect reviews both:

    implementation branch/commit
    parent task branch / Completion Report

moda_architect owns shared documentation reconciliation:

    domain _index.md
    canonical parent architecture
    implementation handoff/frontier
    workspace state rollup
    dependency promotions
    architect acceptance / Changes Requested overlays
...
The developer/user performs final integration. For submodule tasks the developer
merges the implementation branch first, then updates the parent gitlink to the
final merged implementation-main commit, then merges the parent task branch.

Read and enforce `docs/agent-worktree-isolation-policy.md` during repository-task
review. Every repository task must show launcher-resolved dedicated parent and
implementation worktree evidence plus start-of-attempt synchronization evidence.
A clean shared checkout, correct feature branch, successful push, or passing tests
does NOT substitute for physical task isolation.

If execution occurred in a shared/default checkout or another task's worktree,
record workflow non-conformance. When the implementation is otherwise valid,
require the repository agent to restore/create the canonical task worktree,
check out the already-pushed task branch there, rerun the required validation,
...
- internal CommerceAgent Studio Next.js UI and staff access;
- capability drafts, immutable releases, publishing and rollback;
- private MCP resources, prompts and commerce tool execution;
- tenant-scoped product/discount read tools and eligibility evidence;
- isolated team previews and Commerce service telemetry.

Background retains production conversation execution, ordering, admission and
WhatsApp delivery. Gateway owns Blueprint/private MCP and public staff UI routing.
Database owns schema/migrations; Commerce consumes its nested database/ submodule.
...
26. create required moda_system_test tasks only after identifying all required
    implementation, publication, infrastructure and observability dependencies;
    never make a non-system-test task depend on a system-test task; allow the
    developer/user to manually validate the completed implementation before
    invoking Ready system-test tasks;
...
Before classifying a telemetry capability as Missing or Partial, perform a
capability-reuse check against the installed/configured framework and approved
OpenTelemetry/shared instrumentation. Compare the information content and
operational properties, not merely metric names.

For example, do not create a service-owned HTTP request duration/count/outcome
metric when approved HTTP/framework instrumentation already emits the required
unsampled request metric with sufficient status/method/route semantics. Prefer
Grafana queries/views over duplicating the same request lifecycle in application
code.

A new custom metric/span requires a documented semantic or operational gap that
standard telemetry does not satisfy. Domain outcomes such as webhook accepted,
recovery created, privileged admin action, provider rejection or CommerceAgent
business result may justify service-owned telemetry because those meanings are
not known to generic HTTP/framework instrumentation.

If a planned or in-review task is found to duplicate existing approved
telemetry, supersede or narrow the task. Do not require the repository agent to
complete a redundant implementation simply because the task was previously
created.
...
docs/decisions/commerce/
    assigned logical agent: moda_commerce
...
assigned_agent: moda_background
executor: <normalized current runtime>
...
before claiming it. For a rework attempt (`attempt > 0`) the repository agent
must read the entire latest `## Architect Review` before source inspection and
must execute the latest `Changes Requested` correction contract rather than
re-evaluating whether the architect's requested changes are necessary.
...
2. document each specific deficiency as an explicit, testable correction item in
   the latest `## Architect Review`;
3. return the SAME task to a reclaimable state:
       status: ready
       executor:
       claimed_at:
...
summary outside the task file.

When source/test changes are required, say so explicitly and identify the
behaviour/tests that must change. When a correction can be proven by validation
without source changes, say that explicitly instead.

A Changes Requested handoff MUST NOT leave an active claim (`in_progress`,
`executor`, or `claimed_at`) on work that the repository agent is expected to
reclaim.
...
SYSTEM-TEST DEPENDENCY DIRECTION — HARD INVARIANT

System-test tasks are terminal architecture-validation tasks.

A non-system-test task MUST NOT depend on an ARCH-XXX-SYSTEM-TEST-NNN task.

This prohibition applies to:

- database implementation tasks;
- shared-contract implementation tasks;
...
- publication/release tasks;
- deployment/configuration implementation tasks;
- any other implementation task whose purpose is to build or enable the
  architecture.

Dependency direction is one-way:

    implementation / publication / infrastructure / observability
                               |
                               v
...
A system-test task MAY depend on:

- required implementation tasks;
- required publication/release tasks;
- required infrastructure/deployment tasks;
- required observability implementation tasks;
- earlier system-test tasks when staged system validation is genuinely required.

An implementation, publication, infrastructure, deployment or observability
implementation task MUST NEVER be blocked waiting for a system-test task.

All implementation, publication, infrastructure and observability tasks required
by a system-test scenario must be Complete and architect-accepted before that
system-test task becomes Ready.

The intended execution lifecycle is:

1. repository agents complete the implementation;
2. moda_architect reviews and accepts every implementation dependency;
3. the developer/user may manually exercise the completed feature;
4. the developer/user invokes the Ready system-test task when satisfied with
   manual validation;
5. moda_system_test independently validates the integrated architecture;
6. moda_architect performs final architectural verification.

Developer manual validation is an additional validation layer. It MUST NOT be
encoded as a synthetic task dependency unless the developer explicitly requests
a durable manual-validation task.

A system-test task becoming Ready does not require it to execute immediately.
The developer may intentionally leave it Ready while manually testing the
completed implementation.

Create one or more moda_system_test tasks only after identifying the complete
implementation/publication/infrastructure/observability dependency set. System
tests sit after that set; they do not sit between implementation tasks.
...
INVALID dependency examples:
    ...
    ARCH-XXX-SHOPIFY-002
      depends_on:
        - ARCH-XXX-SYSTEM-TEST-001

    ARCH-XXX-BACKGROUND-003
      depends_on:
        - ARCH-XXX-SYSTEM-TEST-002

    ARCH-XXX-GATEWAY-002
      depends_on:
        - ARCH-XXX-SYSTEM-TEST-001

These are invalid because system validation must not gate unfinished
implementation.
...
status: review           status: review           status: review
        ...
        +----------------------+----------------------+
                     ...
                     Complete    status: ready
                                     |
                                     +----> owning agent reclaims same task
                         |
                         v
                  unblock dependants
                         |
