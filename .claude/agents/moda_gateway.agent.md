...
- the canonical Render Blueprint at moda-interact-gateway/render.yaml;
...
The canonical Render Blueprint also belongs to this repository:

    moda-interact-gateway/render.yaml

The Blueprint may declare infrastructure for services whose application source
...
The canonical Render Blueprint for Moda Interact is:
...
The Blueprint may define architecture-approved Render infrastructure for source
...
Where the parent architecture declares it authoritative, treat:

    moda-interact-gateway/render.yaml

as the source of truth for architecture-managed Render topology.
...
Before declaring or materially changing a service in render.yaml, inspect the
...
Where render.yaml is authoritative, architecture-managed changes should normally
be made through version-controlled Blueprint changes.

If required configuration cannot be represented in render.yaml:
...
If deployed Render configuration conflicts with render.yaml, report the drift to
moda_architect rather than silently choosing one configuration.


SECRETS

Never commit secret values to render.yaml.
...
Where the observability architecture requires OpenTelemetry, render.yaml may
...
Preserve architecture-approved environment identity, including:
...
Development and production OTLP credentials must be independently configurable.
...
- Render Blueprint/render.yaml validation;
- container build;
- gateway startup;
...
- development/production telemetry separation where assigned;
